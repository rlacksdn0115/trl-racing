// ---------- content data ----------
  ...
  const drivers = [
    ...
    {num:'05', name:'Taeyun Lee', role:'on road · team leader', a:'Por 935', b:'fav veh', c:'B 2009', d:'Birth',
      bio:'저랑 사귀실 어여쁜 여성분은 디엠 주십쇼', track:'Favorite: Nürburgring', helmetImg: 'taeyun_helmet.png', base: 'MOZA R5', pedal: 'MOZA CRP2', instagram:'t.xyn.09'},
  ];
...
`).join('');
  ...
  // [수정] 스크롤 시 감지하여 로고를 자동으로 스와핑하는 로직 반영
  ...
  navToggle.addEventListener('click', ()=>{
    ...
    // [수정] 모바일 전체 메뉴가 켜지면 무조건 배경이 어두워지므로 다크 로고로 고정, 닫히면 원복
    const logoImg = header.querySelector('.nav-logo img');
    if (logoImg) {
      if (!open) {
